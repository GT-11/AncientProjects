...
{ 
  ...
  DMA_Init(DMA_Streamx, &DMA_InitStructure);//初始化DMA Stream
	...
	DMA_DoubleBufferModeConfig(DMA_Streamx,(u32)mar2,DMA_Memory_0);//双缓冲模式配置
 
  DMA_DoubleBufferModeCmd(DMA_Streamx,ENABLE);//双缓冲模式开启
 
  DMA_ITConfig(DMA_Streamx,DMA_IT_TC,ENABLE);//开启传输完成中断
...
//I2S DMA回调函数指针
void (*adc_tx_callback)(void);	//TX回调函数
void (*adc_rx_callback)(void);	//RX回调函数
//DMA2_Stream0中断服务函数
void DMA2_Stream0_IRQHandler(void)
{      
	if(DMA_GetITStatus(DMA2_Stream0,DMA_IT_TCIF0)==SET)	//DMA1_Stream3,传输完成标志
	{ 
		DMA_ClearITPendingBit(DMA2_Stream0,DMA_IT_TCIF0);	//清除传输完成中断
		wav_second_level_cache();
		gad_second_level_cache();
		//DMA_Rx_Flag = 1;
      //	adc_rx_callback();	//执行回调函数,读取数据等操作在这里面处理  
	}  											 
} 
//I2S开始录音
void ADC_Rec_Start(void)
{   	    
	DMA_Cmd(DMA2_Stream0,ENABLE);//开启DMA TX传输,开始录音	
}
//关闭I2S录音
void ADC_Rec_Stop(void)
{  
	DMA_Cmd(DMA2_Stream0,DISABLE);//关闭DMA,结束录音	 
