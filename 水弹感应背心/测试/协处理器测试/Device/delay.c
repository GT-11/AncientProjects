...
static unsigned int fac_ms=0;//ms延时倍乘数,在ucos下,代表每个节拍的ms数
...
#else
	fac_ms=(unsigned int)fac_us*1000;//非ucos下,代表每个ms需要的systick时钟数   
#endif
}								    
...
void delay_ms(unsigned int nms)
{	 	 
	unsigned char repeat=nms/540;	//这里用540,是考虑到某些客户可能超频使用,
						//比如超频到248M的时候,delay_xms最大只能延时541ms左右了
	unsigned int remain=nms%540;
	while(repeat)
	{
		delay_xms(540);
		repeat--;
