...
void GetPowerMag()
{
 ...
 {	 
	...
	uart_init(115200);	 	//串口初始化为9600
	 ...
	 //////////////////////////////////////////////////////
  ADCDMA_Config(DMA1_Channel1,(u32)&ADC1->DR,(u32)ADC_Value,256);//DMA1通道1,外设为ADC,存储器为ADC_Value,长度5168.  
	...
	DACDMA_Config(DMA2_Channel3,0x40007408,(u32)ADC_Value,256);
...
//    FFT();				//进行 FFT计算
    ...
    cr4_fft_256_stm32(FFT_Out,DAC_Value, 256);
		GetPowerMag();
    //printf("\n");

			for (i=0; i<64; i++)
			{
				for(z=0;z<FFTOut[i+1];z+=10)
				{
					if(l<32)
					box[i][l]='*';
					l++;
