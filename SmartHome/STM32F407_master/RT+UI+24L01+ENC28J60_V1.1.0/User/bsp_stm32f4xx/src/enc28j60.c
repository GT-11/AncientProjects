...
void ENC28J60_SPI3_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;		/* 设为输出口 */
	...
	GPIO_SetBits(GPIOE,GPIO_Pin_6|GPIO_Pin_5);//PG6,7,8上拉				
...
void ENC28J60_Reset(void)
{
	...
	//ENC28J60_SPI3_Init();//SPI3初始化
	//SPI3_SetSpeed(SPI_BaudRatePrescaler_16);	//SPI3 SCK频率为36M/4=9Mhz
	ENC28J60_RST=0;			//复位ENC28J60
	delay1ms();	 delay1ms();	 delay1ms();	 delay1ms();	 delay1ms();	 delay1ms();	 
	ENC28J60_RST=1;			//复位结束				    
	delay1ms();	  delay1ms();	 delay1ms();	 delay1ms();	 delay1ms();	 delay1ms();	 
}
//读取ENC28J60寄存器(带操作码) 
...
u8 ENC28J60_Read_Op(u8 op,u8 addr)
{
	...
	//如果是读取MAC/MII寄存器,则第二次读到的数据才是正确的,见手册29页
 	if(addr&0x80)dat=SPI3_ReadWriteByte(0xFF);
	ENC28J60_CS=1;
	return dat;
}
//读取ENC28J60寄存器(带操作码) 
